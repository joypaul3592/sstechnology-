            ...
            <div className="flex sm:flex-row flex-col items-center justify-center sm:justify-between mb-3">
                ...
                <div className=" flex flex-wrap sm:flex-nowrap items-center sm:gap-10 gap-5 gap-y-1 sm:gap-y-0 mt-3 sm:mt-0 justify-center sm:justify-normal">
                    {menuList.map((menu, index) => (
                        <Link key={index} className={`text-[14px] hover:text-primary font-medium ${pathname === menu.link ? 'text-light' : 'text-gray-300'}`} href={menu.link}>{menu.name}</Link>
                    ))}
                </div>
            </div>
            <Row />
            <span className="block text-sm text-gray-500 text-center dark:text-gray-400 mt-3"><span className=" text-primary">© </span>2023 <span className=" text-primary">SStechnology  </span> . All Rights Reserved.</span>
        </footer >
    )
}
